...
elseif hasleadfield
  ...
  if any(lfdim > lfrank)
    % if LF was backprojected, we perform the orientation reconstruction only in the tangential plane
    ft_warning('at least one of the supplied leadfields does not have full rank, falling back to tangential reconstruction\n');
    istangentialreconstruction = true;
  else
    istangentialreconstruction = false;
  end
else
  ...
  % check if a tangential reconstruction is performed, i.e. if only orientations in the tangential plane are considered in the reconstruction.
  ...
  % for singlesphere MEG headmodels, the leadfield of a radial dipole is zero. Hence it does not make sense to try to reconstruct radial orientation components in this case.
  if strcmp(headmodel.type,'singlesphere') && ft_senstype(sens, 'meg') && ~istangentialreconstruction
